...
import com.flora.root.crypto.core.param.Argon2Parameters;
import com.flora.root.crypto.core.param.ScryptParameters;
import java.lang.SuppressWarnings;
import java.nio.charset.StandardCharsets;

import com.flora.root.crypto.core.bridge.JdkDigest;
import com.flora.root.crypto.core.param.BCryptParameters;
import com.flora.root.crypto.core.param.HkdfParameters;
import com.flora.root.crypto.core.param.Pbkdf2Parameters;
...
class NewCoreKdfVectorTest {
  ...
  void pbkdf2Rfc6070() {
    Pbkdf2DerivationFunction pbkdf2 = new Pbkdf2DerivationFunction(
        new HMac(JdkDigest.of("SHA-1")));
    pbkdf2.init(new Pbkdf2Parameters(
  ...
  void bcryptMatchesCore() {
    ...
    bcrypt.init(new BCryptParameters(password, salt, rounds));
    byte[] out = new byte[dkLen];
    bcrypt.generateBytes(out, 0, dkLen);

    byte[] expected = new byte[dkLen];
    new BCrypt().pbkdf(password, salt, rounds, expected);
