 ...
 * <p>
 * 直接继承 {@link AbstractCacheStore}，自行维护基于 {@link ConcurrentHashMap} 的 KV 与
 * 惰性/主动过期，并在构造时给自己挂载 {@link WTinyLfuEvictionPolicy}（容量约束 + 自适应淘汰）。
 ...
 * {@code EXPIRE} 事件）。{@code capacity <= 0} 时为无界模式：策略处于休眠态、永不淘汰。
 *
 * @param <K> 键类型
 * @param <V> 值类型
 */
public class MemoryCache<K, V> extends AbstractCacheStore<K, V> {
