...
/**
 * 有界缓存契约：在 {@link EvictionConfigableCacheStore}（存储 + 可挂策略）之上叠加尺寸限制。
 * <p>
 ...
 * <p>
 * 事件监听（{@link ObservableCacheStore}）与本接口<b>正交</b>，不再有 is-a 关系：一个缓存可以
 * 有界但不可观测，也可以可观测但无界。
 *
 * @param <K> 键类型
 * @param <V> 值类型
 */
public interface BoundedCacheStore<K, V> extends EvictionConfigableCacheStore<K, V> {
