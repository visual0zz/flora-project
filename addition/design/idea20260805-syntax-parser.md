...
| 句法后顾 | `&<e` / `!<e`（**PEG 扩展**） | 后顾 / 负后顾，不消费；词法层要求长度有界（无界 `*`/`+` 编译期拒绝），文法层无界允许（反向匹配已消费 token 流） |
...
| 句法后顾 `&<e` / `!<e` | ✅ 支持 | PEG 扩展；词法层有界回退重匹配（操作数长度须有界、至少匹配 1 字符），文法层反向匹配已消费 token 流（无界允许） |
