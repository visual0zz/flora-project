...
printf '%b\n' "${CYAN}\$ ./mvnw -pl flora-root,flora-shell,cultivating/flora-sanctum/flora-sanctum-core -am install -DskipTests${NC}"
if ./mvnw -s addition/config/settings.xml -P !osmetes-check -pl flora-root,flora-shell,cultivating/flora-sanctum/flora-sanctum-core -am install -DskipTests; then
...
printf '%b\n' "${CYAN}\$ ./mvnw -pl cultivating/flora-sanctum/flora-sanctum-app clean package -DskipTests${NC}"
if ./mvnw -s addition/config/settings.xml -P !osmetes-check -pl cultivating/flora-sanctum/flora-sanctum-app clean package -DskipTests; then
...
APP=$(ls cultivating/flora-sanctum/flora-sanctum-app/target/flora-sanctum-app-*.jar 2>/dev/null | head -1)
LIB="cultivating/flora-sanctum/flora-sanctum-app/target/lib"
if [ -z "$APP" ] || [ ! -d "$LIB" ]; then
...
call mvnw -s addition/config/settings.xml -P !osmetes-check -pl flora-root,flora-shell,cultivating/flora-sanctum/flora-sanctum-core -am install -DskipTests
if %errorlevel% neq 0 (
...
echo %ESC%[36m$ mvnw -pl cultivating/flora-sanctum/flora-sanctum-app clean package -DskipTests%ESC%[0m
call mvnw -s addition/config/settings.xml -P !osmetes-check -pl cultivating/flora-sanctum/flora-sanctum-app clean package -DskipTests
if %errorlevel% neq 0 (
...
set "APP="
for %%f in (cultivating\flora-sanctum\flora-sanctum-app\target\flora-sanctum-app-*.jar) do set "APP=%%f"
set "LIB=cultivating\flora-sanctum\flora-sanctum-app\target\lib"
if "%APP%"=="" (
