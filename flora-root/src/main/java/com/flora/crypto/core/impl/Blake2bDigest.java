 ...
 */
public final class Blake2bDigest implements ExtendedDigest {
  ...
  public void reset() {
    ...
    // 参数块：digest_length 填入低位，无 key（key_len=0），fanout/tree 默认值
  ...
  /** 压缩当前块；{@code numBytes} 为计入计数器的本块实际消息字节数。 */
  private void compress(int numBytes) {
    // 先累加低 128 位字节计数器，再用于压缩（参考实现顺序）
    long prev = t[0];
    t[0] += numBytes;
    if (t[0] < prev) {
      t[1]++;
