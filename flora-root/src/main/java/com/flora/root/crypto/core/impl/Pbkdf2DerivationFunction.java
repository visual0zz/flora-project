...
import com.flora.root.crypto.core.interfaces.material.param.KeyParameterImpl;
 ...
 * <p>以任意 newcore {@link Mac}（通常 HMAC）为 PRF 迭代：
 * {@code DK = T1 ‖ T2 ‖ ... ‖ Tn}，
 * {@code Ti = U1 ⊕ U2 ⊕ ... ⊕ Uc}，
 * {@code U1 = PRF(P, S ‖ INT32_BE(i))}，
 * {@code Uj = PRF(P, U(j-1))}。</p>
 */
public final class Pbkdf2DerivationFunction implements DerivationFunction {
    ...
    public int generateBytes(byte[] out, int outOff, int len) {
        ...
        for (int i = 1; i <= n; i++) {
            ...
            block[2] = (byte) (i >>> 8);
            block[3] = (byte) i;
            mac.init(new KeyParameterImpl(password));
    ...
    public static final AlgorithmFactory<DerivationFunction> FACTORY = new AlgorithmFactory<>() {
        ...
        public DerivationFunction construct(String algorithmName, AlgorithmComponent... components) {
            ...
            CheckUtil.mustTrue(components.length >= 1 && components[0] instanceof Mac,
                    "PBKDF2 需要注入底层 Mac 组件");
            return new Pbkdf2DerivationFunction((Mac) components[0]);
        }
    };
}
