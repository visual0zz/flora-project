 ...
 * 在实例化、重播种时混入种子材料；生成阶段的附加输入使用 0x02（前）/0x03（后）分隔符。</p>
...
public final class HMacDrbg implements SP80090DRBG {
    ...
    public int generate(byte[] output, byte[] additionalInput, boolean predictionResistant) {
        ...
        if (additionalInput != null && additionalInput.length > 0) {
            macKv((byte) 0x02, additionalInput);
        }
        // temp = V || V || ... 直至足够
        ...
        if (additionalInput != null && additionalInput.length > 0) {
            macKv((byte) 0x03, additionalInput);
        }
        reseedCounter++;
        return output.length * 8;
    }

    private void macKv(byte separator, byte[] data) {
        hmac.init(new KeyParameter(k));
        hmac.update(v, 0, v.length);
