...
> 同步是**可选**能力，仅当库目录命中"在 git 仓库内且基本只有 markdown 对象"时启用（见 04"库形态与 Git 关系"）。否则管理器只读写存储，不操作 git；本文件描述启用同步时的行为。
...
- 仅当启用同步（见 04"库形态与 Git 关系"）时：每次提交自动生成一个 commit（作者信息可配置，如 `sanctum <local>`）；缺 `.git` 则用 `git init`。
- **完全托管仓库判定**：库根**全部是 markdown 文件**即视为完全托管——包括无块的（纯用户正文）和用户手动改动的文件，全部纳入 git 提交与同步（见 04）。满足才提供**同步按钮**。
- 同步前需解锁（DEK 在内存），同步本身不碰明文。
- 未启用同步时：改动直接落盘，无 commit / push / pull。

## 同步流程（自动，不依赖用户介入）

**同步按钮**（仅完全托管仓库提供）触发以下确定步骤（当前实现为**单一远端 origin**）：

```
1. 更新 manifest（写入最新时间戳，重算 MAC）——见 02"仓库时间戳"
2. commit：本次全部改动（含 manifest）一次提交
3. fetch + pull --rebase + 冲突自动解决
4. push 到 origin
```

- **单远端 origin**：当前实现仅与 `origin` 同步（fetch/pull/push），无多远端平级；未来如需多远端在此扩展。
...
2. **同一对象（同一块）被两端改** → 按**块前缀时间戳**（`timestamp:base64` 冒号前数字）仲裁：**大者 wins**（保留时间戳较大的一侧），被覆盖方版本复制到 `<文件名>.conflict` 供核查，不阻塞。
3. **时间戳同值（并发无先后）**：保留本地版本，远端版本记入 `.conflict`，下次打开提示待核查。
...
**自动解决**（按对象类型）：
1. **独立对象文件**（一个文件一个块）：对 ours/theirs 各自解析块前缀 `timestamp:base64` 的冒号前数字（无需解密）；**大者 wins**，写回该文件、`git add` 标记 resolved。
...
- **远端列表（名称、URL、key 引用）**：存为 vault 内 SECRET 对象（随机 UUID，内容自描述，见 05），解锁后扫描定位并应用到 git 远端——多设备一致，无手动重复配置；URL 会泄露托管商，故不落明文。key 引用是统一引用结构 `Ref`（`node:sshKey`=vault 内 SSH 密钥对象 UUID，见 05「统一引用结构 Ref」）。**当前支持单一远端 origin**，同步时仅与其 fetch/pull/push（见"同步流程"）。
...
- 每次提交自动生成一个 commit，使用本地 git 提交（作者信息可配置，如 `sanctum <local>`）。
