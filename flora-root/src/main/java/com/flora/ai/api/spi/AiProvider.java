...
/**
 * AI 提供者 SPI：绑定一种 {@link ApiKind} 协议，按注册模型创建客户端。
 * <p>内置 provider（OpenAI/Anthropic/Gemini 等）由 {@code AiApi} 直接代码注册；
 * 外部新厂商实现本接口并通过 ServiceLoader（{@code META-INF/services/com.flora.ai.spi.AiProvider}）
 ...
 *     public ChatClient createClient(RegisteredModel model) { return new MyClient(model); }
 * }
 * }</pre>
 */
public interface AiProvider {
    ...
    /** 按注册模型创建客户端（实现类按需实现 StreamingClient/JsonClient/MultimodalClient）。 */
    ChatClient createClient(RegisteredModel model);
}
